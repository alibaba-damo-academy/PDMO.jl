import Pkg
Pkg.activate(joinpath(@__DIR__, "..", ".."))

using PDMO

using LinearAlgebra
using SparseArrays
using Random 
Random.seed!(126)

include("DualSVM.jl")
include("../datasets/libsvm.jl")
include("../../src/plotting.jl")


folder_name = "DualSVM"

X, y = load_libsvm_dataset( joinpath(@__DIR__, "../", "datasets/downloaded", "cpusmall_scale"), Float64)  
m, n = size(X)
N = size(y, 1)
Dy = diagm(0 => y)
Q = sparse(Dy * X * X' * Dy)
C= 0.1
maxIter = 10000

mbp = generateDualSVM(Q,  y , C)
# setup ADMM parameters 
param = ADMMParam() 
param.solver = AdaptiveLinearizedSolver(gamma=1.0, r=100.0 , ifSimple=true)
param.logInterval = 100
param.initialRho = 50.0
param.maxIter = maxIter
# run ADMM 
info = runBipartiteADMM(mbp, param)
saveJSONL(info.iterationInfo , "Subroutine 1 (σ=100.0)", folder_name)



mbp = generateDualSVM(Q,  y , C)
# setup ADMM parameters 
param = ADMMParam() 
param.solver = AdaptiveLinearizedSolver(gamma=1.0, r=100.0 , ifSimple=false)
param.logInterval = 100
param.initialRho = 50.0
param.maxIter = maxIter
# run ADMM 
info = runBipartiteADMM(mbp, param)
saveJSONL(info.iterationInfo , "Subroutine 2 (σ=100.0)", folder_name)



mbp = generateDualSVM(Q,  y , C)
param = AdaPDMPlusParam(mbp; t = 10.0)
param.maxIter = maxIter
info = runAdaPDM(
mbp, param
)
saveJSONL_adapdm(info.iterationInfo, "adaPDM+ (t=10.0)", folder_name)


mbp =generateDualSVM(Q,  y , C)
param = MalitskyPockParam(mbp; t = 0.1)
param.maxIter = maxIter
info = runAdaPDM(
mbp, param
)
saveJSONL_adapdm(info.iterationInfo, "Malitsky–Pock (β=0.1)", folder_name)
    

plotting(folder_name , "max" ; smoothing = true) 


