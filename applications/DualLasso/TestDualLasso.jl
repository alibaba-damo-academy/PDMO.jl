...
include("DualLasso.jl")
include("../datasets/libsvm.jl")
include("../../src/plotting.jl")


folder_name =  "Duallasso"

X, y = load_libsvm_dataset( joinpath(@__DIR__, "../", "datasets/downloaded", "housing_scale"), Float64)  
m, n = size(X)
...
mbp = generateDualLasso(X_aug, -y, λ)
...
# setup ADMM parameters 
param = ADMMParam() 
param.solver = AdaptiveLinearizedSolver(gamma=1.0, r=0.01 , ifSimple=false)
param.logInterval = 100
param.initialRho = 50.0
param.maxIter = maxIter
# run ADMM 
info = runBipartiteADMM(mbp, param)
saveJSONL(info.iterationInfo , "Subroutine 2 (σ=0.01)", folder_name)


mbp = generateDualLasso(X_aug, -y, λ)
param = AdaPDMPlusParam(mbp; t = 0.1)
param.maxIter = maxIter
info = runAdaPDM(
mbp, param
)
saveJSONL_adapdm(info.iterationInfo, "adaPDM+ (t=0.1)", folder_name)
