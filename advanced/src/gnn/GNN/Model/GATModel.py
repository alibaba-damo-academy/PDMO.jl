import torch
from torch_geometric.nn import GATv2Conv, dense_mincut_pool, DMoNPooling, MLP, dense_diff_pool,DenseGATConv,GCNConv, BatchNorm, GraphNorm, LayerNorm
...
class GAT_Net(torch.nn.Module):
    def __init__(self, 
        ...
        for _ in range(num_layers_pre):
            self.conv_layers_pre.append(GATv2Conv(hidden_dim, hidden_dim, edge_dim = edge_dim))
        ...
        for _ in range(num_layers_post):
            self.conv_layers_post.append(DenseGATConv(hidden_dim,hidden_dim))
